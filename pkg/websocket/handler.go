...
func (wsConn *wsConn) wsReadLoop(ctx context.Context) {
	for {
		// 读一个message
...
func (wsConn *wsConn) processLoop(ctx context.Context) {
	for {
		select {
...
func (wsConn *wsConn) heartBeatLoop(ctx context.Context) {
	// 启动一个gouroutine发送心跳
	go func() {
		for {
